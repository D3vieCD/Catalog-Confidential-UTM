...
namespace CatalogOnline.API.Controller
{
    ...
    public class CalendarController : ControllerBase
    {
        ...
        [HttpPost]
        public IActionResult CreateEvent([FromBody] CreateCalendarEventDto createData)
        {
            var response = _calendarAction.CreateEventAction(createData);
            if (!response.IsValid) return BadRequest(new { isValid = false, message = response.Message });
            return Ok(new { isValid = true, message = response.Message });
        }
        ...
        public IActionResult GetAllEvents()
        {
            ...
            if (!response.IsValid) return BadRequest(new { isValid = false, message = response.Message });
            return Ok(new { isValid = true, calendarEvents = response.Events });
        }
        ...
        public IActionResult GetEventById([FromRoute] int eventId)
        {
            ...
            if (!response.IsValid) return BadRequest(new { isValid = false, message = response.Message });
            return Ok(new { isValid = true, calendarEvent = response.Event });
        }
        ...
        [HttpPut("{eventId}")]
        public IActionResult UpdateEvent([FromRoute] int eventId, [FromBody] UpdateCalendarEventDto updateData)
        {
